...
<div class="container-fluid p-0">
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-0 pt-4 px-4">
            <div class="row g-4">
                <div class="col-12 col-lg-auto"><h4 class="fw-bold mb-0 text-nowrap">Tasks Report</h4></div>
...
$(document).ready(function () {
    ...
    function fetchTasksReport(page = 1) {
                if (!window.moduleListFilters.bound) { window.moduleListFilters.bound = true; window.moduleListFilters.bind(fetchTasksReport, 'tasksReportSearch'); }
 const url = new URL("{{ route('reports.tasks') }}", window.location.origin); url.searchParams.set('page', page); Object.entries(window.moduleListFilters.values()).forEach(([key,value]) => url.searchParams.set(key,value)); url.searchParams.set('year', $('select[name="year"]').val() || ''); url.searchParams.set('from_date', $('input[name="from_date"]').val() || ''); url.searchParams.set('to_date', $('input[name="to_date"]').val() || ''); if (searchInput.value.trim()) url.searchParams.set('search', searchInput.value.trim()); $.ajax({ url: url.toString(), type: 'GET', dataType: 'json', headers: { 'X-Requested-With': 'XMLHttpRequest', Accept: 'application/json' }, beforeSend: function () { tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-5"><div class="spinner-border text-primary"></div></td></tr>'; }, success: function (res) { if (res.success && res.data) { renderRows(res.data.data || [], res.data); renderPagination(res.data); } }, error: function () { tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-5">Error loading tasks</td></tr>'; paginationContainer.innerHTML = ''; } }); }
    function renderRows(items, meta) { if (!items || !items.length) { tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-5"><div class="text-muted mb-3"><i class="bi bi-inbox display-1 opacity-25"></i></div><p class="text-muted">No data available</p></td></tr>'; return; } tableBody.innerHTML = items.map(function (task, index) { const customerName = escapeHtml(task.customer?.name || task.project?.customer?.name || '-'); const projectName = escapeHtml(task.project?.name || '-'); const taskTitle = escapeHtml(task.title ?? '-'); const taskTypeLabel = escapeHtml(task.task_type || '-'); const statusLabel = escapeHtml(formatLabel(task.status)); const dueDate = escapeHtml(formatDate(task.due_date)); const rowNumber = meta && meta.from ? meta.from + index : index + 1; return `
