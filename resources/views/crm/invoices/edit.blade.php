    ...
    <div class="card shadow-sm border-0 rounded-4 overflow-hidden invoice-form-card">
        <div class="card-header bg-white border-bottom py-3 px-3 px-md-4">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                <div>
                    <h1 class="h4 mb-1 fw-semibold text-dark-blue">Edit Invoice</h1>
                    <p class="text-muted small mb-0">Modify invoice details and billing items.</p>
                </div>
        ...
        <div class="card-body p-3 p-md-4">
            <form action="/api/invoices/{{ $invoice->id }}" method="POST" class="ajax-invoice-form needs-validation" novalidate enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row g-4">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="invoice_name" id="invoice_name" value="{{ $invoice->invoice_name }}" class="form-control @error('invoice_name') is-invalid @enderror" placeholder="Enter invoice name" required>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                            ...
                            <option value="residential" {{ $invoice->type == 'residential' ? 'selected' : '' }}>Residential</option>
                            <option value="commercial" {{ $invoice->type == 'commercial' ? 'selected' : '' }}>Commercial</option>
                            <option value="industrial" {{ $invoice->type == 'industrial' ? 'selected' : '' }}>Industrial</option>
                            <option value="common meter" {{ $invoice->type == 'common meter' ? 'selected' : '' }}>Common Meter</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" min="0" step="1" name="quantity" id="quantity" value="{{ $invoice->quantity }}" class="form-control @error('quantity') is-invalid @enderror" placeholder="Enter kW" required>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" min="0" step="0.01" name="price" id="price" value="{{ $invoice->price }}" class="form-control @error('price') is-invalid @enderror" placeholder="Enter price" required>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-control d-flex align-items-center bg-light border-0" style="min-height: 42px;">
                            <div class="form-check mb-0">
                                <input class="form-check-input" type="checkbox" id="solar_structure_charges_check" value="1" {{ $invoice->solar_structure_charges > 0 ? 'checked' : '' }}>
                                <label class="form-check-label fw-medium" for="solar_structure_charges_check">
                                    Solar Structure Charges
                                </label>
                            </div>
                        </div>
                        <div id="structure-charges-input" style="display: {{ $invoice->solar_structure_charges > 0 ? 'block' : 'none' }}; margin-top: 12px;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" min="0" step="0.01" name="solar_structure_charges" id="solar_structure_charges" value="{{ $invoice->solar_structure_charges }}" class="form-control border-start-0" placeholder="0.00">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="invoice_date" id="invoice_date" value="{{ optional($invoice->invoice_date)->format('Y-m-d') }}" class="form-control @error('invoice_date') is-invalid @enderror">
                    </div>
                    ...
                    <div class="col-md-6 mt-4">
                        ...
                        @if($invoice->attach_file)
                            <div class="mt-2 small">
                                ...
                                <a href="{{ asset('storage/invoices/' . $invoice->attach_file) }}" target="_blank" class="text-primary fw-bold ms-1">
                        ...
                        <textarea name="comment" id="comment" class="form-control @error('comment') is-invalid @enderror" rows="5" placeholder="Add any comments or special notes for this invoice...">{{ $invoice->comment }}</textarea>
                    </div>
                    ...
                    <div class="col-md-6 mt-4">
                        <div class="totals-card rounded-4 p-4 shadow-sm border h-100" style="background: #fdfdfd;">
                            ...
                            <div class="totals-row mb-3 d-flex justify-content-between align-items-center">
                                ...
                                <span class="fw-bold h6 mb-0 text-dark" id="subtotal_display">{{ number_format($invoice->total, 2, '.', '') }}</span>
                            </div>

                            <div class="totals-row mb-3 d-flex justify-content-between align-items-center bg-light p-2 rounded-3">
                                <div class="d-flex align-items-center gap-2">
                                    <label class="switch mb-0">
                                        <input type="checkbox" id="apply_gst" {{ $invoice->gst > 0 ? 'checked' : '' }}>
                                        <span class="slider"></span>
                                    </label>
                                    <span class="text-dark fw-medium">Apply GST</span>
                                </div>
                                <div id="gst_fields_box" style="display: {{ $invoice->gst > 0 ? 'block' : 'none' }};">
                                    <div class="input-group input-group-sm" style="width: 100px;">
                                        <input type="number" id="gst_percent" value="{{ $invoice->gst ?: 18 }}" class="form-control rounded-start-pill">
                            ...
                            <div class="totals-row mb-3 d-flex justify-content-between align-items-center">
                                ...
                                <div class="input-group input-group-sm" style="width: 140px;">
                                    ...
                                    <input type="number" name="discount" id="discount" value="{{ $invoice->discount }}" class="form-control border-start-0 rounded-end-pill">
                                </div>
                            </div>
                            ...
                            <div class="totals-row mb-3 d-flex justify-content-between align-items-center">
                                ...
                                <div class="input-group input-group-sm" style="width: 140px;">
                                    ...
                                    <input type="number" name="subsidy_amount" id="subsidy_amount" value="{{ $invoice->subsidy_amount }}" class="form-control border-start-0 rounded-end-pill">
                                </div>
                            </div>
                            ...
                            <div class="border-top pt-3 mt-3">
                                <div class="totals-row d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="fw-bold text-primary fs-4" id="final_total_display">{{ number_format($invoice->amount, 2, '.', '') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>

                    <input type="hidden" name="total" id="subtotal" value="{{ $invoice->total }}">
                    <input type="hidden" name="final_total" id="final_total" value="{{ $invoice->amount }}">
                    <input type="hidden" name="gst" id="gst" value="{{ $invoice->gst }}">
